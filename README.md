...
- lib/OzocodeGenerator.jar - runnable archive which read files: "out<date>" template.ozocode. and produces ozocode made of out<date> file. 
...
2. picat interpreter (picatWrapper) is invoked with "pr<current_date>.pi" file path and file where "out<date>" where picat output is expected
3. after picat interpreter had finished. "out<date>" is read and parsed.  
...
To be more precise: MapfScenario generates file "pr<data>.pi" and gives it as argument to picatWrapper, which reads file and writes upon it solution file ("out<date>"). This solution file is further used to visualize solution and to generate ozobot program. 
...
Output_file is path to non existing file, to where given predicate should write algorithms result formatted as out<date>.  

## Own Picat Solver (Method A)
It is possible to write own picat solver altorithm. To do that it is needed to write unique predicate in picat_iface.pi with two arguments where from first it reads input file in format "pr<date>.pi" and processed solution writes to file given by second argument, with format "out<date>".
...
Also that predicate needs to have two arguments: first that contains path to file "pr<date>.pi" and second for file where will be written solver output in format of file "out<date>".
...
Example: ./picat mapf_edge_split("Full_Path.pi","Full_path") picat_iface.pi



## Ozocode generator (jar archive)
Main purpose of this module is to convert picat solution file "out<date>" to loadable ozobot program. 
...
```  
   where
   <solution_input_file> is file "out<date>"
